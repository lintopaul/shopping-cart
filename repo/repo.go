...
import (
	...
	"github.com/lintopaul/shopping-cart/db"
	"github.com/lintopaul/shopping-cart/models"
	"github.com/lintopaul/shopping-cart/stats"
)
...
func (r *repo) processOrder(order *models.Order) {
	// ensure order is still completed
...
func (r repo) RequestReversal(orderId string) (*models.Order, error) {
	...
	// place the request on the incoming orders channel
	select {
	case r.incoming <- order:
		r.orders.Upsert(order)
		return &order, nil
